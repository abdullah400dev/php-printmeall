...
class TestComponent extends Component
{
    ...
    public function removevar($value){
      ...
      public function addrproductquery(){
        ...
        //  dd(array_filter($this->variation_array['A1'], 'strlen'));
          $this->validate([
         ...
         foreach($this->variation_array as $key=>$value){
               ...
               foreach($this->variation_array[$key] as $keys=>$values){
                   if(array_key_exists('price', $this->variation_array[$key])){
                      ...
                      $variation->sale_pkr_price = $this->variation_array[$key]['price'];
    ...
    {
        $products = Product::where('id' , '32')->first();
        dd($products->detailss[0]->description);
        
         $cat =  PCategory::all();
        $subcat = RSubCategories::all();
        $product_attributes = ProductAttribute::all();
        return view('livewire.test-component', ['cat'=>$cat, 'subcat'=>$subcat, 'product_attributes'=>$product_attributes]);
    }
}
